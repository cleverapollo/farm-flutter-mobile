...
class _GlobalEntityScreenState extends State<GlobalEntityScreen> {
  ...
  void initState() {
    ...
    var userPortalRoles = context.read<UserInfoCubit>().state.userRoles;
    _isBehave = userPortalRoles.firstWhereOrNull((element) => element.isBehaveRole) != null;
    final isPerform = userPortalRoles.firstWhereOrNull((element) => element.isPerformRole) != null;
    if (!isPerform) {
    ...
    if (isPerform) {
      final isResourceManager = roles.firstWhereOrNull((element) => element.roleName == Constants.resourceManagerRoleName) != null;
